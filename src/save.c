...
} static const sSaveSlotLayout[NUM_SECTORS_PER_SLOT] =
{
    ...
    SAVEBLOCK_CHUNK(struct SaveBlock2, 2), // SECTOR_ID_SAVEBLOCK2_END
    ...
    SAVEBLOCK_CHUNK(struct SaveBlock1, 15), // SECTOR_ID_SAVEBLOCK1_END
...
static void UpdateSaveAddresses(void)
{
    ...
    for (i = SECTOR_ID_PKMN_STORAGE_START; i <= SECTOR_ID_PKMN_STORAGE_END; i++) //setting i to SECTOR_ID_PKMN_STORAGE_START does not match
    {
...
u16 GetSaveBlocksPointersBaseOffset(void)
{
    ...
    for (i = 0; i < NUM_SECTORS_PER_SLOT; i++)
    {
        ...
        if (gReadWriteSector->id == SECTOR_ID_SAVEBLOCK2_START)
