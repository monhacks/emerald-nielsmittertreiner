...
void Overworld_ResetStateAfterTeleport(void)
{
    ...
    RunScriptImmediately(FishermanFerry_EventScript_ResetFerryLocation);
}
...
static u8 GetAdjustedInitialDirection(struct InitialPlayerAvatarState *playerStruct, u8 transitionFlags, u16 metatileBehavior, u8 mapType)
{
    if (FlagGet(FLAG_SYS_CRUISE_MODE) && mapType == MAP_TYPE_OCEAN_ROUTE)
        ...
        return DIR_SOUTH;
    else if (MetatileBehavior_IsSidewaysWarp(metatileBehavior) == TRUE)
...
{
    if (VarGet(VAR_TOURNAMENT_HALL_STATE))
        return FALSE;
...
{
    s8 hours = gSaveBlock2Ptr->inGameClock.hours;
    s8 minutes = gSaveBlock2Ptr->inGameClock.minutes;
...
void CB2_NewGame(void)
{
    ...
    //gFieldCallback = ExecuteTruckSequence; // Disables truck animation and sounds
    gFieldCallback2 = NULL;
