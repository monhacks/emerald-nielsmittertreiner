...
u8 GetSSTidalLocation(s8 *mapGroup, s8 *mapNum, s16 *x, s16 *y)
{
    ...
    switch (*GetVarPointer(VAR_TOURNAMENT_HALL_OPPONENT))
    {
...
void SetTrickHouseNuggetFlag(void)
{
    ...
    u16 flag = FLAG_HIDDEN_ITEM_ACREN_FOREST_POTION;
    *specVar = flag;
    FlagSet(flag);
}
...
bool8 CheckMurenaCityRelicanth(void)
{
    if (GetMonData(&gPlayerParty[0], MON_DATA_SPECIES, 0) == SPECIES_RELICANTH)
        return TRUE;
