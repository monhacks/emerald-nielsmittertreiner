...
void LoadLinkContestPlayerPalettes(void)
{
    ...
    if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK)
    {
        for (i = 0; i < gNumLinkContestPlayers; i++)
        {
            ...
            {
                if (gLinkPlayers[i].gender == MALE)
                    sprite->oam.paletteNum = LoadObjectEventPalette(OBJ_EVENT_PAL_TAG_PLAYER_MALE);
                else
                    sprite->oam.paletteNum = LoadObjectEventPalette(OBJ_EVENT_PAL_TAG_PLAYER_FEMALE);
            }
        }
    }
}
