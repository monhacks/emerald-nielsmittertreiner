...
static void Task_NewGameBirchSpeech_LoadTailAfterGenderAndName(u8);
...
void CB2_NewGameBirchSpeech(void)
{
    ResetTasks();
    SetVBlankCallback(VBlankCB_MainMenu);
    SetMainCallback2(CB2_MainMenu);
...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_KECLEON);
...
static void Task_NewGameBirchSpeech_LoadTailAfterGenderAndName(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        LoadTail(140, 50);
...
{
    return CreateMonPicSprite_Affine(SPECIES_KECLEON, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
