...
void CB2_InitPokenav2(void)
{
    ...
    bench = EndBenchmark();
    mgba_printf(MGBA_LOG_DEBUG, "%d", bench);
...
static void InitEventWindows(void)
{
    ...
    FillWindowPixelBuffer(WIN_AGENDA_QUESTS_PROGRESS, PIXEL_FILL(4));
    ...
    if (sPokenav2Ptr->timeColonRedraw)
    {
        ...
        if (sPokenav2Ptr->timeColonInvisible)
        {
            if (gSaveBlock2Ptr->is24HClockMode)
                ...
                FillWindowPixelRect(WIN_AGENDA_DATE_TIME, PIXEL_FILL(0), 21, 16, 6, 12);
        }
